...
void main() {
  ...
  testWidgets('ItemPositions: single item list then update to 10 items list',
      (WidgetTester tester) async {
    tester.binding.window.devicePixelRatioTestValue = 1.0;
    tester.binding.window.physicalSizeTestValue =
        const Size(screenWidth, screenHeight);
    ...
    expect(find.text('Item 0'), findsOneWidget);
    expect(find.text('Separator 0'), findsNothing);
    expect(itemPositionsListener.itemPositions.value.length, 1);
    expect(
        itemPositionsListener.itemPositions.value
            .firstWhere((position) => position.index == 0)
            .itemLeadingEdge,
    ...
    expect(
        itemPositionsListener.itemPositions.value
            .firstWhere((position) => position.index == 0)
            .itemTrailingEdge,
        _screenProportion(numberOfItems: 1, numberOfSeparators: 0));

    //update list to 10 items
    itemCount.value = 10;
    await tester.pumpAndSettle();
